import 'package:cloud_firestore/cloud_firestore.dart';
import 'package:revistabike_app/domain/datasource/modalities_datasources.dart';
import 'package:revistabike_app/domain/entities/00-entities.dart';
import 'package:revistabike_app/infrastructure/mappers/00-mappers.dart';
import 'package:revistabike_app/infrastructure/models/00-models.dart';

class ModalitiesFirebaseDatasource extends ModalitiesDataSource {
  FirebaseFirestore db = FirebaseFirestore.instance;

  @override
  Future<List<Modality>> getAllModalitiesAvailable() async {
    //! Se realiza el llamado al api de firebase para obtener los resultados
    QuerySnapshot<Map<String, dynamic>> snapshot = await db
        .collection("Modalidades")
        .where("Habilitado", isEqualTo: true)
        .get();

    //! Se castea el resultado de firebase al modelo
    final modalitiesResponse = snapshot.docs
        .map((e) => ModalityResponse.fromDocumentSnapshot(e))
        .toList();

    //! Se castea el resultado de firebase a la entidad de mi programa
    List<Modality> modalities = modalitiesResponse
        .map((e) => ModalityMapper.modalityToEntity(e))
        .toList();
    modalities.sort((a, b) => a.priority.compareTo(b.priority));
    return modalities;
  }
}
